        ...
        for message in consumer:
            ...
            if dbid is not None:
                rows = oracle_pool.execute_query_and_fetchall(
                                sql_list['EMPTY_SUBPARTITIONS_TABLES_SELECT_DB_PASS_DSN'],
                                dbid=dbid)

                if len(rows) == 1:
                    oracle_pool.execute_query_and_commit(
                            sql_list['EMPTY_SUBPARTITIONS_TABLES_START_DB'],
                            dbid=dbid)
                ...
                for row in rows:
                    if len(rows) == 1:
                        ...
                        for row_db in rows_db:
                            oracle_pool.execute_query_and_commit(
                                                sql_list['EMPTY_SUBPARTITIONS_TABLES_INSERT_RESULTS'],
                                                dbid=dbid,
                                                owner=row_db[0],
                        ...
                        oracle_pool.execute_query_and_commit(
                                                sql_list['EMPTY_SUBPARTITIONS_TABLES_INSERT_RES_CONN'],
                                                dbid=dbid,
                                                status=str(status),
